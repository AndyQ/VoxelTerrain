...
//                tex_fact[4] = texfactor( 0, hmap_height)
                ...
                tex_fact[0] = texfactor( 256, hmap_height, 64)
                tex_fact[1] = texfactor( 192, hmap_height, 64)
                tex_fact[2] = texfactor( 128, hmap_height, 64)
                tex_fact[3] = texfactor( 100, hmap_height, 20)
                tex_fact[4] = texfactor( 0, hmap_height, 100)
                ...
                final_tex[x,y] = Color(r: UInt8(min(r+50, 255)), g: UInt8(min(g+50, 255)), b: UInt8(min(b+50, 255)))
            }
        }
        return final_tex
    }
    
    func texfactor(_ h1: Float, _ h2 : Float, _ range: Float) -> Float {
        var percent : Float = Float((range - abs(h1 - h2)) / range)
